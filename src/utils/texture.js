...
function createVideoEl (material, src, width, height) {
  ...
  el.height = height;
  if (el !== this.videoEl) {
    el.setAttribute('webkit-playsinline', 'true');  // To support inline videos in iOS webapps.
    el.autoplay = true;
    el.loop = true;
    el.crossOrigin = true;
...
function fixVideoAttributes (videoEl) {
  ...
  if (!videoEl.hasAttribute('webkit-playsinline')) {
    videoEl.setAttribute('webkit-playsinline', 'true');  // To support inline videos in iOS webapps.
  }
  return videoEl;
}
